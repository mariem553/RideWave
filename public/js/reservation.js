/* ═══════════════════════════════════════════════════════════════
   ...
   // Exposé sur window pour que le script inline de reservation.html puisse y accéder
   ...
   async function initReservationPage() {
     ...
     // Pré-remplir les champs client avec les données de l'utilisateur connecté
     try {
       ...
       if (userStr) {
         ...
         if (user.nom) {
           ...
           // Premier mot = nom de famille, le reste = prénom
           if (lastNameEl)  lastNameEl.value  = parts[0] || '';
           if (firstNameEl) firstNameEl.value = parts.slice(1).join(' ') || parts[0] || '';
         }
         if (user.email) {
       ...
       console.warn('[reservation] Impossible de pré-remplir les infos client :', e);
     }
