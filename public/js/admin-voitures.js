// Données statiques — remplacées par API en Phase 2
let voitures = [
...
function renderList() {
  ...
  if (data.length === 0) {
    body.innerHTML = `<div style="text-align:center;padding:32px;color:#5a5a65;font-size:12px;">Aucune voiture trouvée</div>`
    return
  }
  body.innerHTML = data.map(v => `
    <div class="list-row ${selected?.id === v.id ? 'active' : ''}" onclick="selectVoiture(${v.id})">
      <img class="list-thumb" src="${v.photo}" onerror="this.src=''" />
      <div class="list-info">
      ...
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;">
        <div class="list-price">${v.prix_jour} DT</div>
...
function renderDetail() {
  ...
  document.getElementById('detail-panel').innerHTML = `
    <div class="detail-img-wrap">
      <img src="${v.photo}" onerror="this.style.display='none'" />
    ...
    <div class="detail-price-wrap">
      ...
      <div class="detail-price-unit">DT / jour</div>
    </div>
...
function saveVoiture() {
  ...
  const prix   = parseInt(document.getElementById('form-prix').value)
...
function deleteVoiture(id) {
  ...
  renderList()
  if (selected) renderDetail()
  else document.getElementById('detail-panel').innerHTML = `<div class="empty"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(201,169,98,0.25)" stroke-width="1.5"><path d="M5 17H3v-5l2-5h14l2 5v5h-2"/><circle cx="7.5" cy="17.5" r="2.5"/><circle cx="16.5" cy="17.5" r="2.5"/></svg>Sélectionnez une voiture</div>`
}
